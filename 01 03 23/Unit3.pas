...
type
  ...
  private
    current: string; //определение переменных как в C++, даже искать информацию о том как объявлять поля не пришлось. лучше бы всё это проходили на C++ а не вот этого вот непонятное что-то.
...
{$R *.dfm}
  ...
  var s: string; //а тут уже решили добавить определение переменной. ну что за непостоянные люди...
  recenter: TMenuItem; //устраиваем преколы с кастами
  begin
    //recenter := TMenuItem(Sender);
    recenter := Sender as TMenuItem;
    s := recenter.Caption;
    image1.picture.LoadFromFile(s);
    //recent1.Caption := current; //ага, значит это глобальная переменная. и никто об этом не сказал. а ведь даже никто и не объяснил как создавать глобальные переменные.
    checktorecentness(); //ужас какой из-за задания пришлось вытоврить...
    current := s;
  end;

  procedure TForm3.open1Click(Sender: TObject);
  ////ну вот почему никто не предупредил что это за переменная? почему нельзя было хотя бы просто вначале написать "создадим поле класса типа string"? почему я должен сам догадываться о таких мелочах? я понимаю что можно было бы найти что такое поле, но чтобы искать проблему "почему в моём коде куча ошибок?" это что за гении писали код?
  //var current: string; //ну тупа наугад. нигде не сказано откуда эта переменная взялась, но по её использованию можно догадаться что она типа string. --какие-то не очень умные люди решили скинуть код не объяснив что за что отвечает или что откуда берётся... могли хотя бы комментарии написать, а то, просто скопировать и скинуть - глупость.
  begin
    if OpenPictureDialog1.Execute() then
    begin
      if current <> '' then checktorecentness(); //ну и что на этот раз такое "current"?
